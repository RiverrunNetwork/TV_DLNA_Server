...
{
public:
    static const NPT_String* GetOWN(const NPT_HttpMessage& message) {
        return message.GetHeaders().GetHeaderValue("OWN");
    }

    static NPT_Result SetOWN(NPT_HttpMessage& message,
                            const char*      own) {
        return message.GetHeaders().SetHeader("OWN", own);
    }
    // methods
    ...
    static NPT_Result SetCustom(NPT_HttpMessage& message,
                             const char*      nts) {
        return message.GetHeaders().SetHeader("CustomValue", nts);
    }
