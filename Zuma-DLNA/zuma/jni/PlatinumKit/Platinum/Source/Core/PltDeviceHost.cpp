                                       ...
                                       const NPT_SocketAddress* addr /* = NULL */)
{
    ...
    response.GetHeaders().SetHeader("showVR",  device->m_showVR);
    response.GetHeaders().SetHeader("version", device->m_version);
    response.GetHeaders().SetHeader("BaoFengPort", "21367");
    response.GetHeaders().SetHeader("BaoFengVRPort", "21368");
    // UPnP 1.1 CONFIGID.UPNP.ORG header
    if (device->m_ConfigId > 0) {
